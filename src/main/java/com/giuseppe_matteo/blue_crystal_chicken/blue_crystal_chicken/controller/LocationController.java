...
public class LocationController {
    ...
    // GET /api/locations/{id}/station-token — token-stazione della sede (solo Admin)
    ...
    // GET /api/locations/by-station-token — la stazione (Cucina/Tabellone/Cassa) scopre
    // la propria sede dal token (header X-Station-Token). Pubblico: è il token la credenziale.
    ...
    public ResponseEntity<?> byStationToken(
        ...
        var loc = locationService.findByStationToken(token);
        if (loc == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("error", "Token-stazione mancante o non valido"));
        }
        String city = loc.getAddress() != null ? loc.getAddress().getCity() : "";
