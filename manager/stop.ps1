# ============================================================================
#  Manager (web) — STOP
#  Ferma il dev server di Vite: prima il processo salvato in .manager.pid,
...
if (Test-Path ".manager.pid") {
    $pidValue = (Get-Content ".manager.pid" | Select-Object -First 1).Trim()
    if ($pidValue) {
        taskkill /PID $pidValue /T /F | Out-Null
        $killed = $true
    }
    Remove-Item ".manager.pid" -Force
}
...
if ($killed) {
    Write-Host "Manager fermato (porta $port liberata)." -ForegroundColor Green
} else {
    Write-Host "Nessun manager in esecuzione sulla porta $port." -ForegroundColor Yellow
}
