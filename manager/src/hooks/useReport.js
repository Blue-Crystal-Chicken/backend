...
export function useReport() {
  // MANAGER: bloccato sulla propria sede
  const scope = managerLocationId();
  const [range, setRange]           = useState("7");
  const [locationId, setLocationIdRaw] = useState(scope);
  ...
  useEffect(() => {
    locationService.getAll().then((d) => {
      const list = Array.isArray(d) ? d : [];
      setLocations(scope ? list.filter((l) => l.id === scope) : list);
    }).catch(() => {});
