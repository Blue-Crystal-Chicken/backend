  ...
  const login = useCallback(async (email, password) => {
    ...
    try {
      ...
      const userData = {
        ...
        locationId: data.locationId ?? null,       // sede del manager (dal backend)
        locationName: data.locationName ?? null,   // nome sede (per badge/scoping)
        name: data.name,
        surname: data.surname
      };

      // Questo è il pannello MANAGER: solo gli account con ruolo MANAGER possono
      // accedere. Un admin (o altro ruolo) deve usare il proprio pannello.
      if (userData.role !== "ROLE_MANAGER") {
        throw new Error(
          "Accesso riservato ai Manager. Usa le credenziali del tuo account manager di sede."
        );
      }
