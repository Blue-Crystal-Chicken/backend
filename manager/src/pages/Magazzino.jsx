...
import { managerLocationId, managerLocationName } from "../lib/managerScope";

// ============================================================================
//  Magazzino del MANAGER = scorte della SUA sede (locationId da auth).
//  Il manager può aggiungere/aggiornare/rimuovere scorte della propria sede.
//  (Non è il catalogo globale ingredienti: quello resta dell'Admin.)
// ============================================================================
...
export default function Magazzino() {
  const locationId = managerLocationId();
  const sedeName = managerLocationName();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [st, ings] = await Promise.all([
        ingredientService.getStockByLocation(locationId),
      ...
      console.error("Errore caricamento scorte sede:", err);
    } finally {
      setLoading(false);
    }
  }, [locationId]);
  ...
  const handleSaveQty = async (li) => {
    ...
    try {
      await ingredientService.updateStock(locationId, ingredientId, Number(raw));
      fetchData();
    } catch (err) {
  ...
  const handleRemove = async (li) => {
    if (!window.confirm(`Rimuovere "${ingNameOf(li)}" dalle scorte della tua sede?`)) return;
    try {
      await ingredientService.removeStock(locationId, ingIdOf(li));
      fetchData();
    } catch (err) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (existing) {
        await ingredientService.updateStock(locationId, ingredientId, (existing.quantity ?? 0) + delta);
      } else {
        await ingredientService.addStock(locationId, ingredientId, delta);
      }
      setIsAddOpen(false);
  ...
  if (!locationId) {
    return (
      <div className="p-[28px_32px] max-w-[1440px] mx-auto text-[#f0f4ff]">
        <div className="border border-dashed border-[#fbbf24]/30 bg-[#fbbf24]/5 text-[#fbbf24] rounded-[18px] p-8 text-center text-[14px] font-semibold">
          Nessuna sede assegnata a questo manager. Contatta l'Admin.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-[28px_32px] max-w-[1440px] mx-auto text-[#f0f4ff] bg-[#0b0c10] min-h-screen">
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          <h1 className="font-['Syne'] text-[26px] font-extrabold tracking-tight mb-1">Magazzino</h1>
          <p className="text-[13px] text-[#8b92a8] uppercase tracking-wide">
            Scorte della sede {sedeName ? `— ${sedeName}` : `#${locationId}`}
          </p>
        </div>
