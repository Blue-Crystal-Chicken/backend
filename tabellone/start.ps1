...
if ($existing) {
    Write-Host "Il tabellone risulta gia' attivo sulla porta $port." -ForegroundColor Yellow
...
# --- Avvio in background ------------------------------------------------------
Write-Host "Avvio del tabellone sulla porta $port..." -ForegroundColor Cyan
...
$proc.Id | Out-File -FilePath ".tabellone.pid" -Encoding ascii

Start-Sleep -Seconds 2
Write-Host ""
Write-Host "Tabellone avviato (PID $($proc.Id))." -ForegroundColor Green
Write-Host "  -> http://localhost:$port"
