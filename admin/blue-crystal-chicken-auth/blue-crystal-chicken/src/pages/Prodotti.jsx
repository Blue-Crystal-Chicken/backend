...
export default function ProductDashboard() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await productService.updateProduct(currentId, formData, selectedFile);
            } else {
                await productService.createProduct(formData, selectedFile);
                // Notifica push (non blocca: fire-and-forget)
                const res = await pushNotificationService.notifyEvent({
                    title: "Nuovo prodotto",
                    body: `"${formData.name}" è stato aggiunto al catalogo.`,
                    type: "NEW_PRODUCT",
                    payload: { name: formData.name, category: formData.category },
                });
                if (res.ok) notifyPushSent("Notifica push inviata: nuovo prodotto");
                else notifyPushError(`Notifica prodotto non inviata: ${res.error}`);
            }
    ...
    const handleDelete = async (id, name) => {
        if (window.confirm(`Eliminare definitivamente il prodotto "${name}"?`)) {
            try {
    ...
    const handleMenuSubmit = async (e) => {
        ...
        setSavingMenu(true);
        try {
            await menuService.createWithProducts({ ...menuForm, products: selected }, menuFile);
            // Notifica push (non blocca: fire-and-forget)
            const res = await pushNotificationService.notifyEvent({
                title: "Nuovo menu",
                body: `Il menu "${menuForm.name}" è disponibile.`,
                type: "NEW_MENU",
                payload: { name: menuForm.name },
            });
            if (res.ok) notifyPushSent("Notifica push inviata: nuovo menu");
            else notifyPushError(`Notifica menu non inviata: ${res.error}`);
            setIsMenuModalOpen(false);
            fetchAllData();
        } catch (err) {
            alert("Creazione menu fallita: " + (err?.message || "errore sconosciuto"));
        } finally {
            setSavingMenu(false);
        }
    };
    ...
    const handleMenuDelete = async (id, name) => {
        if (window.confirm(`Eliminare definitivamente il menu "${name}"?`)) {
            try {
                await menuService.remove(id);
                fetchAllData();
            } catch (err) {
                alert("Errore durante l'eliminazione del menu: " + err.message);
            }
        }
    };
