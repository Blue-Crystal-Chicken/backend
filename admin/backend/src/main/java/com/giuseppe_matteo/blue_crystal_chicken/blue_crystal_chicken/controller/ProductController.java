...
public class ProductController {
    ...
    public ProductController(ProductService productService, NotificationPublisher notificationPublisher) {
    ...
    // Lista prodotti per ID categoria — usato dalla Cassa (counter).
    // categoryId == 0 → tutti i prodotti (gestito nel service).
    @GetMapping("/v1/category/id/{categoryId}")
    @Operation(summary = "Lista prodotti filtrato per id categoria")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<?> getProductsByCategoryId(@PathVariable Long categoryId) {
        log.info("GET /api/products/v1/category/id/{}", categoryId);
        try {
            return ResponseEntity.ok(productService.getProductsByCategoryId(categoryId));
        } catch (Exception e) {
            log.error("GET /api/products/v1/category/id/{} - ERROR: {}", categoryId, e.getMessage());
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> updateProduct(@PathVariable Long id, @ModelAttribute ProductRequest product) {
        ...
        try {
            ...
            notificationPublisher.publish("event.product.updated", "PRODUCT_UPDATED", "INFO", "Prodotti",
                    "Prodotto modificato", "Il prodotto \"" + updated.getName() + "\" è stato aggiornato.");
    ...
    public ResponseEntity<?> deleteProduct(@PathVariable Long id) {
        ...
        try {
            ...
            notificationPublisher.publish("event.product.deleted", "PRODUCT_DELETED", "WARNING", "Prodotti",
                    "Prodotto eliminato", "Il prodotto \"" + deleted.getName() + "\" è stato rimosso dal catalogo.");
    ...
    public ResponseEntity<?> createProduct(@ModelAttribute ProductRequest product) {
        ...
        try {
            ...
            notificationPublisher.publish("event.product.created", "PRODUCT_CREATED", "SUCCESS", "Prodotti",
                    "Nuovo prodotto", "Il prodotto \"" + created.getName() + "\" è stato aggiunto al catalogo.");
